...
class GroupSearchStore extends ReduceStore<State> {
  ...
  _updateWindowUrl(state: State) {
    if (state.findGroupsArgs) {
      ...
      const windowUrl: string = urls.constructWithQueryString(
        urls.section(Section.FindGroups),
        urlArgs
      );
  ...
  _initializeFilters(state: State, findGroupsArgs: FindGroupsArgs): State {
    ...
    state = this._addLocationToState(
      state,
      locationRadiusFromString(findGroupsArgs.locationRadius)
    );
  ...
  _getTagCategoryParams(state: State, category: string): ?string {
    const tags = this.getSelectedTags(state).filter(
