...
class GroupTagContainer extends React.Component<{||}, State> {
  ...
  static calculateState(prevState: State): State {
    ...
    pillConfigs = pillConfigs.concat(GroupSearchStore.getSelectedTags().toJS().map((tag: TagDefinition) => {
      return {
        label: tag.display_name,
        closeAction: () => GroupSearchDispatcher.dispatch({type: 'REMOVE_TAG', tag: tag})
      };
    }));
    ...
    if(locationRadius && locationRadius.latitude && locationRadius.longitude) {
      pillConfigs.push({
        ...
        closeAction: () => GroupSearchDispatcher.dispatch({type: 'SET_LOCATION', locationRadius: null})
      });
    }
