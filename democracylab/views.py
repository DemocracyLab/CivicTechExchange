...
def login_view(request, provider=None):
    ...
    if request.method == "POST":
        ...
        if "prevPageArgs" in request.POST and len(request.POST["prevPageArgs"]) > 0:
            prev_page_args_string = (
                request.POST["prevPageArgs"].strip("'\"").replace("\\", "")
            )
        user = authenticate(username=email.lower(), password=password)
        if user is not None and user.is_authenticated:
            login(request, user)
            prev_page_args = (
                json.loads(prev_page_args_string) if prev_page_args_string else None
            )
            redirect_url = (
                "/"
                if prev_page.strip("/") == ""
            ...
            back_args = {"prev": prev_page, "username": email}
    ...
    if provider in provider_ids:
        ...
        if "prevPageArgs" in request.GET and len(request.GET["prevPageArgs"]) > 0:
            prev_page_args_string = (
                request.GET["prevPageArgs"].strip("'\"").replace("\\", "")
            )
        prev_page_args = (
            json.loads(prev_page_args_string) if prev_page_args_string else None
        )
...
def signup(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            last_name = form.cleaned_data.get("last_name")
            email = form.cleaned_data.get("email")
            back_args = {"firstname": first_name, "lastname": last_name, "email": email}
            return redirect(section_url(FrontEndSection.SignUp, back_args))
    else:
...
def password_reset(request):
    ...
    messages.add_message(
        request,
        messages.INFO,
        "Your request to reset your password has been received. If an account with that email exists, an email will be sent with a link to reset your password.",
    )
