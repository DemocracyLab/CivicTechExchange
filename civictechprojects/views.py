...
from common.helpers.s3 import (
    presign_s3_upload,
    user_has_permission_for_s3_file,
    delete_s3_file,
)
from common.helpers.tags import get_tags_by_category, get_tag_dictionary
from common.helpers.form_helpers import (
    is_co_owner_or_staff,
    is_co_owner,
    is_co_owner_or_owner,
    is_creator_or_staff,
    is_creator,
)
from .forms import (
...
from democracylab.emails import (
    send_to_project_owners,
    send_to_project_volunteer,
    HtmlEmailTemplate,
    send_volunteer_application_email,
    send_volunteer_conclude_email,
    notify_project_owners_volunteer_renewed_email,
    notify_project_owners_volunteer_concluded_email,
    notify_project_owners_project_approved,
    contact_democracylab_email,
    send_to_group_owners,
    send_group_project_invitation_email,
    notify_group_owners_group_approved,
    notify_event_owners_event_approved,
    notify_rsvped_volunteer,
    notify_rsvp_cancellation,
    notify_rsvp_for_project_owner,
    notify_rsvp_cancellation_for_project_owner,
    notify_rsvp_cancellation_for_event_owner,
...
from common.helpers.redirectors import (
    redirect_by,
    InvalidArgumentsRedirector,
    DirtyUrlsRedirector,
    DeprecatedUrlsRedirector,
)
from common.helpers.user_helpers import (
    get_my_projects,
    get_my_groups,
    get_my_events,
    get_user_context,
...
@cache_page(
    1200
)  # cache duration in seconds, cache_page docs: https://docs.djangoproject.com/en/2.1/topics/cache/#the-per-view-cache
def group_tags_counts(request):
    # Get all groups
    all_groups = Group.objects.all()
    # Get Groups issue areas
    group_issues = list(
        map(lambda group: group.get_project_issue_areas(with_counts=False), all_groups)
    )
...
def approve_group(request, group_id):
    ...
    if group is not None:
        if user.is_staff:
            ...
            return redirect(
                section_url(FrontEndSection.AboutGroup, {"id": str(group.id)})
            )
        else:
            return HttpResponseForbidden()
    else:
...
def get_event(request, event_id):
    ...
    return (
        JsonResponse(event.hydrate_to_json(get_request_contributor(request)))
        if event
...
def get_event_project(request, event_id, project_id):
    ...
    return (
        JsonResponse(event_project.hydrate_to_json(get_request_contributor(request)))
        if event_project
        else HttpResponse(status=404)
    )
...
def event_project_edit(request, event_id, project_id):
    ...
    event_project = EventProjectCreationForm.create_or_edit_event_project(
        request, event_id, project_id
    )
...
def rsvp_for_event_project(request, event_id, project_id):
    ...
    if rsvp is None:
        ...
        rsvp = RSVPVolunteerRelation.create(
            event, user, request.data["isRemote"], request.data["locationTimeZone"]
        )
    ...
    if project is not None:
        if user.is_staff:
            ...
            message = (
                ...
                else "Project Restored from Archive"
            )
...
def approve_event(request, event_id):
    ...
    if event is not None:
        if user.is_staff:
            ...
            return redirect(
                section_url(FrontEndSection.AboutEvent, {"id": str(event.id)})
            )
        else:
            return HttpResponseForbidden()
    else:
...
def index(*args, **kwargs):
    ...
    if (
        ...
        return redirect(
            section_url(FrontEndSection.AddUserDetails, {"provider": account.provider})
        )

    redirect_result = redirect_by(
        [InvalidArgumentsRedirector, DirtyUrlsRedirector, DeprecatedUrlsRedirector],
    ...
    if settings.GOOGLE_TAGS_ID:
        ...
        context["googleTagsHeadScript"] = loader.render_to_string(
            "scripts/google_tag_manager_snippet_head.txt", google_tag_context
        )
        context["googleTagsBodyScript"] = loader.render_to_string(
            "scripts/google_tag_manager_snippet_body.txt", google_tag_context
        )
    ...
    if request.user.is_authenticated:
        ...
        context[
            "volunteeringUpForRenewal"
        ] = contributor.is_up_for_volunteering_renewal()
        context["QIQO_IFRAME_URL"] = get_user_qiqo_iframe(contributor, request)

        # TODO: Get thumbnail from cached user
        thumbnail = ProjectFile.objects.filter(
            file_user=request.user.id, file_category=FileCategory.THUMBNAIL.value
        ).first()
        if thumbnail:
...
def get_site_stats(request):
    ...
    stats = {
        ...
        "dlVolunteerCount": active_volunteers.filter(
            is_approved=True, project__id=settings.DLAB_PROJECT_ID
        ).count(),
    }
...
def upcoming_events(request):
    ...
    events = Event.objects.filter(
        ...
        is_searchable=True,
        is_private=False,
        event_date_end__gt=timezone.now(),
    )
    return JsonResponse(
        {
            "events": [
                event.hydrate_to_tile_json()
                for event in events.order_by("event_date_start")[:event_count]
            ]
        }
    )
...
def limited_listings(request):
    ...
    approved_projects = ProjectPosition.objects.filter(
        position_project__is_searchable=True
...
def volunteer_operation_is_authorized(request, volunteer_relation):
    ...
    authorized_usernames = [volunteer_relation.project.project_creator.username] + list(
        map(
            lambda co: co.volunteer.username,
...
def contact_project_owner(request, project_id):
    ...
    email_subject = "{firstname} {lastname} sent a message to {project}".format(
        firstname=user.first_name, lastname=user.last_name, project=project.project_name
    )
    email_template = (
...
def contact_project_volunteers(request, project_id):
    ...
    email_subject = "{project}: {subject}".format(
        project=project.project_name, subject=subject
    )
    email_template = (
        HtmlEmailTemplate(use_signature=False)
        .header(
    ...
    if not is_creator(user, project):
        ...
        send_to_project_owners(
            project, user, email_subject, email_template, include_co_owners=False
        )
    ...
    for volunteer in volunteers:
        ...
        send_to_project_volunteer(
            volunteer, email_subject, email_template, cc_owners=False
        )
...
def contact_event_project_volunteers(request, event_id, project_id):
    ...
    email_subject = "[{event}] {project}: {subject}".format(
        event=event_project.event.event_name, project=project_name, subject=subject
    )
    email_template = (
        HtmlEmailTemplate(use_signature=False)
        .header(
    ...
    for volunteer in volunteers:
        send_to_event_project_volunteer(
            event_project.project, volunteer, email_subject, email_template
        )
...
def renew_volunteering_with_project(request, application_id):
    ...
    notify_project_owners_volunteer_renewed_email(
        volunteer_relation, request.data["message"]
    )
    return HttpResponse(status=200)
...
def accept_project_volunteer(request, application_id):
    ...
    if not request.user.is_authenticated:
        return redirect(
            section_url(FrontEndSection.LogIn, {"prev": request.get_full_path()})
        )

    volunteer_relation = VolunteerRelation.objects.get(id=application_id)
    about_project_url = section_url(
        FrontEndSection.AboutProject, {"id": str(volunteer_relation.project.id)}
    )
    if volunteer_relation.is_approved:
        messages.add_message(
            request, messages.ERROR, "This volunteer has already been approved."
        )
...
def reject_project_volunteer(request, application_id):
    ...
    if volunteer_operation_is_authorized(request, volunteer_relation):
        ...
        send_to_project_volunteer(
            volunteer_relation=volunteer_relation,
...
def dismiss_project_volunteer(request, application_id):
    ...
    if volunteer_operation_is_authorized(request, volunteer_relation):
        ...
        email_template = HtmlEmailTemplate().paragraph(
            'Thank you for contributing to {project_name}. Your volunteer work for the project has ended. "{message}"'.format(
...
def leave_project(request, project_id):
    volunteer_relation = VolunteerRelation.objects.filter(
        project_id=project_id, volunteer_id=request.user.id
    ).first()
    if request.user.id == volunteer_relation.volunteer.id:
        ...
        if len(message) > 0:
            ...
            email_template = HtmlEmailTemplate().paragraph(
                "{volunteer_name} is leaving {project_name} for unspecified reasons.".format(
                    volunteer_name=volunteer_relation.volunteer.full_name(),
                    project_name=volunteer_relation.project.project_name,
                )
            )
...
def contact_group_owner(request, group_id):
    ...
    email_subject = "{firstname} {lastname} would like to connect with {group}".format(
        firstname=user.first_name, lastname=user.last_name, group=group.group_name
    )
    email_template = (
        HtmlEmailTemplate(use_signature=False)
...
def accept_group_invitation(request, invite_id):
    ...
    if project_relation.is_approved:
        messages.add_message(
            request, messages.ERROR, "The project is already part of the group."
        )
        return redirect(about_project_url)
    ...
    if is_co_owner_or_owner(user, project):
        ...
        if request.method == "GET":
            messages.add_message(
                request,
                messages.SUCCESS,
                "Your project is now part of the group "
                + project_relation.relationship_group.group_name,
            )
            return redirect(about_project_url)
        else:
            return HttpResponse(status=200)
        ...
        messages.add_message(
            request,
            messages.ERROR,
            "You do not have permission to accept this group invitation.",
        )
        return redirect(about_project_url)
...
def reject_group_invitation(request, invite_id):
    ...
    if is_co_owner_or_owner(user, project):
        ...
        messages.add_message(
            request,
            messages.ERROR,
            "You do not have permission to reject this group invitation.",
        )
        return redirect(about_project_url)
...
def project_favorite(request, project_id):
    ...
    if existing_fav is not None:
        print(
            "Favoriting project:{project} by user:{user}".format(
        ...
        print(
            "Favorite already exists for project:{project}, user:{user}".format(
                project=project.id, user=user.id
            )
        )
...
def project_unfavorite(request, project_id):
    ...
    if existing_fav is not None:
        print(
            "Unfavoriting project:{project} by user:{user}".format(
                project=project.id, user=user.id
            )
        )
        existing_fav.delete()
        ...
        print(
            "Can't Unfavorite project:{project} by user:{user}".format(
                project=project.id, user=user.id
            )
        )
        return HttpResponse(status=400)
...
def contact_democracylab(request):
    ...
    if r.json()["success"]:
        ...
        interest_flags = list(
            filter(
                lambda key: request.data[key] and isinstance(request.data[key], bool),
                request.data.keys(),
            )
        )
        contact_democracylab_email(
            first_name, last_name, email_addr, message, company_name, interest_flags
        )
        return HttpResponse(status=200)
...
@api_view()
def team(request):
    response = {"board_of_directors": settings.BOARD_OF_DIRECTORS}
...
def get_testimonials(request, category=None):
    ...
    return JsonResponse(
        list(map(lambda t: t.to_json(), testimonials.order_by("-priority"))), safe=False
    )
...
def qiqo_webhook(request):
    ...
    if action == "meeting.participant_joined":
        existing_participant = EventConferenceRoomParticipant.get(
            existing_room, participant_id
        )
        if not existing_participant:
            ...
            print(
                "User {user_id} has already joined room {room_id}".format(
                    user_id=participant_id, room_id=room_id
                )
            )
        ...
        if existing_participant:
            print(
                "User {user_id} has left room {room_id}".format(
                    user_id=participant_id, room_id=room_id
                )
            )
            ...
            print(
                "User {user_id} is not in {room_id}".format(
                    user_id=participant_id, room_id=room_id
                )
            )
            return HttpResponse(status=302)
