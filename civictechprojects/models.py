...
from civictechprojects.caching.cache import (
    ProjectCache,
    GroupCache,
    EventCache,
    ProjectSearchTagsCache,
    EventProjectCache,
)
...
class Project(Archived):
    project_creator = models.ForeignKey(
        Contributor, related_name="created_projects", on_delete=models.CASCADE
    )
    ...
    project_organization_type = TaggableManager(
        blank=True, through=TaggedOrganizationType
    )
    ...
    project_location_coords = PointField(
        null=True, blank=True, srid=4326, default="POINT EMPTY"
    )
    ...
    event_created_from = models.ForeignKey(
        "Event",
        ...
        if slug is not None:
            ...
            if _slug.isnumeric():
                ...
                project = Project.objects.filter(
                    project_slug=_slug
                ).first() or NameRecord.get_project(_slug)
    ...
    def _hydrate_to_json(self):
        ...
        group_relationships = ProjectRelationship.objects.filter(
            relationship_project=self
        ).exclude(relationship_group=None)
        commits = ProjectCommit.objects.filter(commit_project=self.id).order_by(
            "-commit_date"
        )[:20]
        trello_actions = TrelloAction.objects.filter(action_project=self.id).order_by(
            "-action_date"
        )[:20]
        actions = sorted(
            chain(commits, trello_actions),
            reverse=True,
            key=lambda x: getattr(x, "commit_date", getattr(x, "action_date", "")),
        )
        project = {
            "project_id": self.id,
            ...
            "project_organization": Tag.hydrate_to_json(
                self.id, list(self.project_organization.all().values())
            ),
            "project_organization_type": Tag.hydrate_to_json(
            ...
            "project_stage": Tag.hydrate_to_json(
                self.id, list(self.project_stage.all().values())
            ),
            "project_technologies": Tag.hydrate_to_json(
                self.id, list(self.project_technologies.all().values())
            ),
            "project_positions": list(
                map(lambda position: position.to_json(), positions)
            ),
            ...
            "project_groups": list(
                map(lambda gr: gr.hydrate_to_list_json(), group_relationships)
            ),
            "project_events": list(
                map(lambda er: er.hydrate_to_tile_json(), self.get_project_events())
            ),
            "project_owners": [self.project_creator.hydrate_to_tile_json()],
            "project_volunteers": list(
                map(lambda volunteer: volunteer.to_json(), volunteers)
            ),
    ...
    def get_project_files(self):
        return ProjectFile.objects.filter(
            file_project=self, file_user=None, file_group=None, file_event=None
        )
    ...
    def get_project_groups(self):
        project_relationships = ProjectRelationship.objects.filter(
            relationship_project=self.id
        )
        groups_ids = list(
            map(lambda pr: pr.relationship_group.id, project_relationships)
        )
...
class Group(Archived):
    group_creator = models.ForeignKey(
        Contributor, related_name="group_creator", on_delete=models.CASCADE
    )
    ...
    group_location_coords = PointField(
        null=True, blank=True, srid=4326, default="POINT EMPTY"
    )
        ...
        if slug is not None:
            ...
            if _slug.isnumeric():
                ...
                group = Group.objects.filter(
                    group_slug=_slug
                ).first() or NameRecord.get_group(_slug)
    ...
    def _hydrate_to_json(self):
        ...
        if len(projects) > 0:
            group["group_issue_areas"] = self.get_project_issue_areas(
                with_counts=True, project_relationships=projects
            )
    ...
    def get_group_links(self):
        return ProjectLink.objects.filter(
            link_group=self, link_project=None, link_event=None, link_user=None
        )

    def get_group_files(self):
        return ProjectFile.objects.filter(
            file_group=self, file_project=None, file_user=None, file_event=None
        )

    def get_project_issue_areas(self, with_counts, project_relationships=None):
        if project_relationships is None:
            project_relationships = ProjectRelationship.objects.filter(
                relationship_group=self.id
            )
        all_issue_areas = flatten(
            list(
                map(
                    lambda p: p.relationship_project.project_issue_area.all().values(),
                    project_relationships,
                )
            )
        )
        all_issue_area_names = list(
            map(lambda issue_tag: issue_tag["name"], all_issue_areas)
        )
        if with_counts:
    ...
    def get_group_project_relationships(self, approved_only=True):
        ...
        if approved_only:
            project_relationships = project_relationships.filter(
                is_approved=True, relationship_project__is_searchable=True
            )
    ...
    def get_group_projects(self, approved_only=True):
        project_ids = list(
            map(
                lambda pr: pr.relationship_project.id,
                self.get_group_project_relationships(approved_only=approved_only),
            )
        )
...
class Event(Archived):
    ...
    event_creator = models.ForeignKey(
        Contributor, related_name="event_creator", on_delete=models.CASCADE
    )
    ...
    event_legacy_organization = TaggableManager(
        blank=True, through=TaggedEventOrganization
    )
    ...
    def hydrate_to_json(self, user=None):
        hydrated = EventCache.get(self) or EventCache.refresh(
            self, self._hydrate_to_json()
        )
    ...
    def _hydrate_to_json(self):
        ...
        projects = self.get_event_projects()
        ...
        event = {
            ...
            "event_date_modified": self.event_date_modified.isoformat(
                timespec="seconds"
            ),
            ...
            "event_time_zones": list(
                map(lambda time_zone: time_zone.hydrate_to_json(), time_zones)
            ),
            ...
            "event_legacy_organization": Tag.hydrate_to_json(
                self.id, list(self.event_legacy_organization.all().values())
            ),
            "event_project_count": projects.count(),
    ...
    def get_event_project_relationships(self, approved_only=True):
        project_relationships = ProjectRelationship.objects.filter(
    ...
    def get_event_files(self):
        return ProjectFile.objects.filter(
            file_event=self, file_project=None, file_user=None, file_group=None
        )
    ...
    def get_url(self):
        return section_url(
            FrontEndSection.AboutEvent, {"id": self.event_slug or self.id}
        )
        ...
        if slug is not None:
            ...
            if _slug.isnumeric():
                ...
                event = Event.objects.filter(
                    event_slug=_slug
                ).first() or NameRecord.get_event(_slug)
    ...
    def get_issue_areas(self):
        ...
        project_ids = list(
            map(
                lambda relationship: relationship.relationship_project.id,
                project_relationships,
            )
        )
    ...
    def get_linked_projects(self):
        projects = Project.objects.filter(
            project_events__event=self,
            project_events__deleted=False,
            is_searchable=True,
        )
        return projects
...
class EventLocationTimeZone(models.Model):
    event = models.ForeignKey(
        Event, related_name="event_time_zones", on_delete=models.CASCADE
    )
    ...
    def create(event, event_location_time_zone_json):
        time_zone = EventLocationTimeZone(event=event, **event_location_time_zone_json)
        return time_zone
...
class EventProject(Archived):
    event = models.ForeignKey(
        Event, related_name="event_projects", on_delete=models.CASCADE
    )
    project = models.ForeignKey(
        Project, related_name="project_events", on_delete=models.CASCADE
    )
    creator = models.ForeignKey(
        Contributor, related_name="created_event_projects", on_delete=models.CASCADE
    )
    event_time_zone = models.ForeignKey(
        EventLocationTimeZone,
        related_name="time_zone_event_projects",
        null=True,
        blank=True,
    ...
    def hydrate_to_json(self, user=None):
        hydrated = EventProjectCache.get(self) or EventProjectCache.refresh(
            self, self._hydrate_to_json()
        )
    ...
    def _hydrate_to_json(self):
        ...
        event_json = keys_subset(
            self.event.hydrate_to_json(),
            [
                "event_id",
                "event_name",
                "event_slug",
        ...
        project_json = keys_subset(
            self.project.hydrate_to_json(),
            [
                "project_id",
                "project_name",
                "project_thumbnail",
        ...
        event_project_json = merge_dicts(
            event_json,
            project_json,
            {
                ...
                "event_project_positions": list(
                    map(lambda position: position.to_json(), positions)
                ),
                "event_project_volunteers": list(
                    map(lambda rsvp: rsvp.to_json(), rsvps)
                ),
        ...
        if self.event_time_zone is not None:
            event_project_json[
                "event_time_zone"
            ] = self.event_time_zone.hydrate_to_json()
        ...
        if event_room is not None:
            ...
            event_project_json[
                "event_conference_participants"
            ] = event_room.participant_count()

        return event_project_json

    def hydrate_to_list_json(self):
        return keys_subset(
            self.hydrate_to_json(),
            [
                "event_project_id",
                "event_id",
                "event_name",
    ...
    def get_event_project_links(self):
        return ProjectLink.objects.filter(
            link_project=self.project,
            link_event=self.event,
            link_group=None,
            link_user=None,
        )
    ...
    def get_project_positions(self):
        return ProjectPosition.objects.filter(
            position_project=self.project, position_event=self.event
        ).order_by("order_number")
    ...
    def get_url(self):
        return section_url(
            FrontEndSection.AboutEventProject,
            {
                "event_id": self.event.event_slug or self.event.id,
...
class NameRecord(models.Model):
    event = models.ForeignKey(
        Event, related_name="old_slugs", blank=True, null=True, on_delete=models.CASCADE
    )
    group = models.ForeignKey(
        Group, related_name="old_slugs", blank=True, null=True, on_delete=models.CASCADE
    )
    project = models.ForeignKey(
        Project,
        related_name="old_slugs",
        blank=True,
        null=True,
        on_delete=models.CASCADE,
    )
...
class ProjectRelationship(models.Model):
    relationship_project = models.ForeignKey(
        Project,
    ...
    relationship_event = models.ForeignKey(
        Event,
        related_name="relationships",
        blank=True,
        null=True,
...
class ProjectCommit(models.Model):
    commit_project = models.ForeignKey(
        Project, related_name="commits", blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return "({repo}) {sha}: {title}".format(
            repo=self.repo_name, sha=self.commit_sha[:6], title=self.commit_title
        )

    @staticmethod
    def create(project, repo_name, branch_name, github_json):
        commit_sha = github_json["sha"]
        existing_commit = ProjectCommit.objects.filter(
            commit_sha=commit_sha, commit_project=project.id
        )
...
class TrelloAction(models.Model):
    ...
    def get_avatar_url(self):
        ...
        if self.member_avatar_base_url:
            return "{base}/{size}.png".format(
                base=self.member_avatar_base_url, size=avatar_size
            )
        else:
            return ""
...
class ProjectLink(models.Model):
    link_project = models.ForeignKey(
        Project, related_name="links", blank=True, null=True, on_delete=models.CASCADE
    )
    link_group = models.ForeignKey(
        Group, related_name="links", blank=True, null=True, on_delete=models.CASCADE
    )
    link_event = models.ForeignKey(
        Event, related_name="links", blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return "{id}: {name} - {url}".format(
            id=self.id, name=self.link_name, url=self.link_url
        )
    ...
    def merge_changes(owner, links):
        ...
        for link_json in links:
            ...
            if not link.id:
                ProjectLink.create(
                    owner, link.link_url, link.link_name, link.link_visibility
                ).save()
            else:
...
class ProjectPosition(models.Model):
    position_project = models.ForeignKey(
        Project, related_name="positions", on_delete=models.CASCADE
    )
    position_event = models.ForeignKey(
        Event, related_name="positions", blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    def to_json(self):
        return {
            ...
            "roleTag": Tag.hydrate_to_json(self.id, self.position_role.all().values())[
                0
            ],
    ...
    def merge_changes(owner, positions):
        ...
        updated_positions_ids = set(
            map(lambda position: position["id"], updated_positions)
        )
        existing_positions = owner.get_project_positions()
        existing_positions_ids = set(
            map(lambda position: position.id, existing_positions)
        )
        existing_projects_by_id = {
            position.id: position for position in existing_positions
        }
        ...
        for updated_position_json in updated_positions:
            ProjectPosition.update_from_json(
                existing_projects_by_id[updated_position_json["id"]],
                updated_position_json,
            )

        for deleted_position_id in deleted_position_ids:
            ProjectPosition.delete_position(
                existing_projects_by_id[deleted_position_id]
            )
...
class ProjectFile(models.Model):
    ...
    file_project = models.ForeignKey(
        Project, related_name="files", blank=True, null=True, on_delete=models.CASCADE
    )
    file_user = models.ForeignKey(
        Contributor,
        related_name="files",
        blank=True,
        null=True,
    ...
    file_group = models.ForeignKey(
        Group, related_name="files", blank=True, null=True, on_delete=models.CASCADE
    )
    file_event = models.ForeignKey(
        Event, related_name="files", blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    def merge_changes(owner, files):
        ...
        if type(owner) is Project:
            old_files = list(
                owner.get_project_files()
                .filter(file_category=FileCategory.ETC.value)
                .values()
            )
        elif type(owner) is Group:
            old_files = list(
                owner.get_group_files()
                .filter(file_category=FileCategory.ETC.value)
                .values()
            )
        elif type(owner) is Event:
            old_files = list(
                owner.get_event_files()
                .filter(file_category=FileCategory.ETC.value.value)
                .values()
            )
        elif type(owner) is EventProject:
            old_files = list(
                owner.get_event_project_files()
                .filter(file_category=FileCategory.ETC.value.value)
                .values()
            )
        else:
            old_files = list(
                owner.get_user_files()
                .filter(file_category=FileCategory.ETC.value)
                .values()
            )

        for file in added_files:
            ProjectFile.from_json(
                owner=owner, file_category=FileCategory.ETC, file_json=file
            ).save()
    ...
    def replace_single_file(owner, file_category, file_json, new_file_category=None):
        ...
        if type(owner) is Project:
            existing_file = (
                owner.get_project_files()
                .filter(file_category=file_category.value)
                .first()
            )
        elif type(owner) is Group:
            existing_file = (
            ...
            existing_file = (
                owner.get_user_files().filter(file_category=file_category.value).first()
            )
...
class ProjectFavorite(models.Model):
    ...
    link_user = models.ForeignKey(
        Contributor, related_name="favorites", on_delete=models.CASCADE
    )
...
class VolunteerRelation(Archived):
    project = models.ForeignKey(
        Project, related_name="volunteer_relations", on_delete=models.CASCADE
    )
    volunteer = models.ForeignKey(
        Contributor, related_name="volunteer_relations", on_delete=models.CASCADE
    )
    ...
    application_date = models.DateTimeField(
        auto_now=False, null=False, default=timezone.now
    )
    ...
    re_enroll_last_reminder_date = models.DateTimeField(
    ...
    def get_by_project(project, active=True):
        return VolunteerRelation.objects.filter(
            project_id=project.id, is_approved=active, deleted=not active
        )


class TaggedRSVPVolunteerRole(TaggedItemBase):
    content_object = models.ForeignKey(
        "RSVPVolunteerRelation", on_delete=models.CASCADE
    )


class RSVPVolunteerRelation(Archived):
    event = models.ForeignKey(
        Event, related_name="rsvp_volunteers", on_delete=models.CASCADE
    )
    volunteer = models.ForeignKey(
        Contributor, related_name="rsvp_events", on_delete=models.CASCADE
    )
    event_project = models.ForeignKey(
        EventProject,
        related_name="rsvp_volunteers",
        blank=True,
        null=True,
        on_delete=models.CASCADE,
    )
    time_zone = models.ForeignKey(
        EventLocationTimeZone,
        related_name="time_zone_volunteers",
        null=True,
        on_delete=models.CASCADE,
    )
    is_remote = models.BooleanField(blank=True, null=True)
    ...
    def __str__(self):
        return "Event: {event}, User: {user}".format(
            event=str(self.event.event_name), user=str(self.volunteer.email)
        )
    ...
    def to_json(self):
        ...
        volunteer_json = {
            ...
            "roleTag": Tag.hydrate_to_json(
                volunteer.id, list(self.role.all().values())
            ),
        }
    ...
    def create(
        ...
        time_zone = (
            ...
            if event_location_time_zone_json
            else None
        )
    ...
    def get_for_event_volunteer(event: Event, volunteer: Contributor):
        return RSVPVolunteerRelation.objects.filter(
            event=event.id, volunteer=volunteer.id
        ).first()

    @staticmethod
    def get_for_event_project(event_project: EventProject, volunteer: Contributor):
        return RSVPVolunteerRelation.objects.filter(
            event_project=event_project.id, volunteer=volunteer.id
        ).first()
...
class EventConferenceRoom(models.Model):
    ...
    event = models.ForeignKey(
        Event, related_name="conference_rooms", on_delete=models.CASCADE
    )
    event_project = models.ForeignKey(
        EventProject,
        related_name="event_conference_rooms",
        blank=True,
        null=True,
        on_delete=models.CASCADE,
    )
    ...
    def __str__(self):
        event_prefix = (
            self.event_project.__str__() if self.event_project else self.event.__str__()
        )
        return "{event}: {room_number}".format(
            event=event_prefix, room_number=self.room_number
        )
    ...
    def create(
        ...
        room = (
            EventConferenceRoom.get_event_project_room(event_project)
    ...
    def create_for_entity(event: Event, event_project: EventProject = None):
        ...
        if qiqo_event_id is not None:
            ...
            for room_activation_json in activate_response:
                ...
                print(
                    "Created room {room} for {entity}".format(
                        room=zoom_id, entity=(event_project or event).__str__()
                    )
                )
                EventConferenceRoom.create(
                    event, zoom_id, join_url, admin_url, event_project
                )


class EventConferenceRoomParticipant(models.Model):
    room = models.ForeignKey(
        EventConferenceRoom, related_name="participants", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return "{room} - {name}({id})".format(
            room=self.room.__str__(), name=self.zoom_user_name, id=self.zoom_user_id
        )

    @staticmethod
    def get(room: EventConferenceRoom, zoom_user_id: str):
        return EventConferenceRoomParticipant.objects.filter(
            room=room, zoom_user_id=zoom_user_id
        ).first()
